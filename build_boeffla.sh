...
if [ "kltekor" == "$KERNEL_VARIANT" ]; then
	...
	make -C $(pwd) O=output_kor arch=arm boeffla_kltekor_defconfig VARIANT_DEFCONFIG=boeffla_defconfig_variant && make -j$NUM_CPUS -C $(pwd) O=output_kor
	if [ -e output_kor/arch/arm/boot/zImage ]; then
		...
		echo -e $COLOR_GREEN"\n cleaning...\n"$COLOR_NEUTRAL
		cd .. && mv anykernel_boeffla/$KERNEL_NAME-* release_boeffla/ && rm anykernel_boeffla/zImage && rm anykernel_boeffla/dtb && rm -r anykernel_boeffla/modules/
